package com.picelo.endpoint.service.userdetails;

import com.picelo.endpoint.service.user.UserService;
...
public class ReactiveUserAccessService implements ReactiveUserDetailsService {
	...
	public Mono<UserDetails> findByUsername(final String username) {
		return getUser(username)
				.switchIfEmpty(Mono.defer(
						() -> Mono.just(username).filter(name -> name.contains("@")).flatMap(this::getUserByEmail)))
	...
	public Mono<UserAccessModel> getUserByEmail(final String email) {
		...
		return Mono.justOrEmpty(this.userSecurityTable.index("email_index").query(queryEnhancedRequest).stream()
				.flatMap(page -> page.items().stream()).findFirst());

	}

	public Mono<UserAccessModel> addUserSecurity(final UserService.NewUserRequest request, final String id) {
		var userSecurityModel = UserAccessModel.builder().username(request.username()).email(request.email()).id(id)
				.password(passwordEncoder.encode(request.password())).enabled(true).credentialsNonExpired(true)
				.accountNonExpired(true).accountNonLocked(true).accountLoggedIn(false)
	...
	public Mono<UserAccessModel> updateUserAccess(final UserAccessModel user) {
		return Mono.just(userSecurityTable.updateItem(
				UpdateItemEnhancedRequest.builder(UserAccessModel.class).item(user).ignoreNulls(true).build()));
	}
