...
import static com.scryer.endpoint.service.tag.ConsolidateUtil.getCombinedTags;
import static com.scryer.endpoint.service.tag.ConsolidateUtil.getSubtractedTags;
...
public class ImageService {
	...
	public Flux<ImageSrcModel> getImageSrcForFolder(final String folderId) {
		...
		return Flux
				.fromStream(imageSrcTable.index("folder_index")
						.query(QueryEnhancedRequest.builder().queryConditional(queryConditional).attributesToProject()
								.build())
	...
	public Mono<ImageSrcModel> addTagsToImageSrc(final ImageSrcModel imageSrc, List<String> tags) {
		return updateImageSrc(ImageSrcModel.builder().id(imageSrc.getId()).lastModified(System.currentTimeMillis())
				.tags(getCombinedTags(imageSrc, tags)).build());
	}

	public Mono<ImageSrcModel> deleteImageSrcTags(final ImageSrcModel imageSrc, List<String> tags) {
		return updateImageSrc(ImageSrcModel.builder().id(imageSrc.getId()).lastModified(System.currentTimeMillis())
				.tags(getSubtractedTags(imageSrc, tags)).build());
	}
	...
	public Mono<ImageSrcModel> addImageSrc(final ImageSrcModel imageSrc) {
		return Mono.justOrEmpty(imageSrcTable
				.putItemWithResponse(PutItemEnhancedRequest.builder(ImageSrcModel.class).item(imageSrc).build())
	...
	private Mono<ImageSrcModel> createImageSrc(final String imageId, final String name, final String[] type,
			final String size, final String path, final String folderId, final String userId,
			final String alternateSize, final String alternateSource) {
		long currentTime = System.currentTimeMillis();
		return Mono.just(ImageSrcModel.builder().id(imageId).userId(userId).type(type[0]).name(name).size(size)
				.source(new ImageBaseIdentifier("url", path)).createDate(currentTime).lastModified(currentTime)
				.parentFolderId(folderId)
				.alternateSizes(Map.of(alternateSize, new ImageBaseIdentifier("url", alternateSource)))
				.imageRankings(List.of()).tags(List.of()).build());
