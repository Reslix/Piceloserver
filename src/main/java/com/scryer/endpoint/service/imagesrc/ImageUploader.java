...
class ImageUploader {
	...
	@Autowired
	public ImageUploader(final S3Client s3Client, final String s3BucketName) {
	...
	public Mono<String> uploadImage(final String id, final String size, final byte[] image, final String[] type) {
		...
		var request = PutObjectRequest.builder().bucket(s3BucketName).key(key).contentType(type[0]).build();
