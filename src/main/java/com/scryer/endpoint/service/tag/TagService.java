...
public class TagService {
				...
				.switchIfEmpty(Mono.defer(() -> {
					...
					var tagMono = Mono
							.justOrEmpty(tagTable.index("tag_index")
									...
									.stream().flatMap(page -> page.items().stream()).findFirst().map(tagTable::getItem))
							.cache();
	...
	public Flux<TagModel> getUserTags(final String userId) {
		...
		var tagFlux = Mono.justOrEmpty(tagTable.index("userId_index")
				.query(QueryEnhancedRequest.builder().queryConditional(queryConditional).build()).stream()
				.flatMap(page -> page.items().stream())).flatMapIterable(Stream::toList);
	...
	public Mono<TagModel> addNewTag(final String name, final String userId) {
		...
		return Mono
				.justOrEmpty(tagTable.putItemWithResponse(PutItemEnhancedRequest.builder(TagModel.class)
						.item(TagModel.builder().id(id).name(name).imageIds(List.of()).imageRankingIds(List.of())
								.userId(userId).build())
						.build()).attributes())
