...
public class JWTManager {
	...
	public String createJwtAccess(final String username, final String userId) {
		return Jwts.builder().signWith(KEY_PAIR.getPrivate(), SignatureAlgorithm.forSigningKey(KEY_PAIR.getPrivate()))
				.setSubject(username).claim(Claims.ID, userId).setIssuer("identity")
				.setExpiration(Date.from(Instant.now().plus(Duration.ofHours(8)))).setIssuedAt(Date.from(Instant.now()))
				.compact();
	}
	...
	public String createJwtRefresh(final String username, final String userId) {
		return Jwts.builder().signWith(KEY_PAIR.getPrivate(), SignatureAlgorithm.forSigningKey(KEY_PAIR.getPrivate()))
				.setSubject(username).claim(Claims.ID, userId).setIssuer("identity")
				.setExpiration(Date.from(Instant.now().plus(Duration.ofDays(2)))).setIssuedAt(Date.from(Instant.now()))
				.compact();
	}
	...
	public Jws<Claims> validateJwt(final String jwt) {
		return Jwts.parserBuilder().setSigningKey(KEY_PAIR.getPublic()).build().parseClaimsJws(jwt);
	}
