...
public class LoginHandler {
	...
	@Autowired
	public LoginHandler(final ReactiveUserAccessService reactiveUserAccessService, final JWTManager jwtManager,
		...
		return usernameMono.then(userMono)
				.flatMap(
						user -> reactiveUserAccessService.updateUserAccess(UserAccessModel.builder()
								.username(user.getUsername()).credentialsNonExpired(user.isCredentialsNonExpired())
								.accountNonExpired(user.isAccountNonExpired())
								.accountNonLocked(user.isAccountNonLocked()).enabled(user.isEnabled())
								.accountLoggedIn(true).build()))
				.flatMap(user -> ServerResponse.ok()
						.cookie(ResponseCookie
								.from("accessToken", jwtManager.createJwtAccess(user.getUsername(), user.getId()))
								.domain(cookieDomain).httpOnly(true).path("/").sameSite(
										Cookie.SameSite.LAX.attributeValue())
								.build())
						.cookie(ResponseCookie
								.from("refreshToken", jwtManager.createJwtRefresh(user.getUsername(), user.getId()))
	...
	public Mono<ServerResponse> logout(final ServerRequest serverRequest) {
		...
		var logoutMono = userAccessMono.map(user -> reactiveUserAccessService.updateUserAccess(UserAccessModel.builder()
				...
				.enabled(user.isEnabled()).accountLoggedIn(false).build()));
